...
pub fn validity(excel: &Range<DataType>) -> Option<Vec<String>> {
    ...
    for (index, row) in excel.rows().enumerate() {
        // if the title cell is empty
        if index > 0 {
            break
        }
        let mut titles: Vec<String> = vec!["Filename", "last_revised_just_now", "file_index_later_calculated", "index_later_calculated", "fileindex+index", "Data"].into_iter().map(|f| f.to_string()).collect();
        for cell in row {
            if cell.is_empty() {
                return None
            }
...
/// Returns a matrix of rows for each query instance being found 
/// Examples
/// ```rust
/// let example_response = vec!["name_of_query_found_in_row", "remaining row data"];
/// println!("This would be an example of a response");
/// ```
pub fn filter_rows(row: &[DataType], query: &HashSet<String>) -> Option<Vec<Vec<String>>> {
    ...
    for data in query {
        ...
        if rows.contains(data) {
            let mut new_row: Vec<String> = vec!["Filename", "last_revised_just_now", "file_index_later_calculated", "index", "fileindex+index", data].into_iter().map(|f| f.to_string()).collect();
